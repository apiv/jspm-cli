...
exports.download = function(pkg, options, installDeps, installPeerDeps) {
  ...
  var preloadCallbacks = [depsCallback];
  var postloadCallbacks = [depsCallback];

  function preloadResolve(deps) {
    preloadCallbacks.forEach(function(cb) {
      cb(deps);
    });
  }
  function postloadResolve(deps) {
  ...
  .then(function(dirExists) {
    ...
    .then(function(pkgHash) {
      // if the package exists locally, stat the package@x.y.z.json file to see if it has been changed
      // if the stat is different to the hash file stat, then add the altered package@x.y.z.json contents
      // to the overrides as an override, and note that we have done this
      if (pkgHash) {
        return Promise.all([
          asp(fs.stat)(downloadDir + '.json'),
          asp(fs.stat)(path.resolve(downloadDir, '.jspm-hash'))
        ])
        .then(function(stats) {
          if (stats[0].mtime.getTime() - stats[1].mtime.getTime() > 2000) {
            ...
            .then(function(pkgConfig) {
              ...
              ui.log('ok', 'The package configuration file `' + path.relative(path.dirname(config.pjsonPath), downloadDir + '.json') + '` has been edited manually. To avoid overwriting the change, it has been added to the package.json overrides.');
            });
          }
          return pkgHash;
        }, function(err) {
...
function getDepRanges(pkg, packageConfig) {
  ...
  var newDepRanges = processDeps(packageConfig.dependencies, packageConfig.registry, pkg.exactName);
...
function derivePackageConfig(pkg, packageConfig, override) {
  ...
  if (override) {
    ...
    // overrides extend to the first level
    var stringified;
    for (var p in override) {
      if (typeof override[p] == 'object' && !(override[p] instanceof Array)) {
        if (typeof packageConfig.jspm[p] != 'object')
          packageConfig.jspm[p] = {};
        for (var q in override[p]) {
          stringified = JSON.stringify(override[p][q]);
          if (JSON.stringify(packageConfig.jspm[p][q]) !== stringified && (!packageConfig[p] || JSON.stringify(packageConfig[p][q]) !== stringified))
            packageConfig.jspm[p][q] = JSON.parse(stringified);
          else
            delete override[p][q];
        }
      }
      else {
        stringified = JSON.stringify(override[p]);
...
function processPackage(pkg, dir, packageConfig, linked) {
  ...
  return (linked ? Promise.resolve() : filterIgnoreAndFiles(dir, packageConfig.ignore, packageConfig.files))
  .then(function() {
